...
for i in range(len(logits)):
    prob_dict = {f"prob_{j}": probs[i, top_indices[i, j]] for j in range(top_k)} # 选取top_k概率的值
    prob_dict["row_id"] = test.row_id.values[i] # 添加row_id
    prob_dict["top_classes"] = " ".join(le.inverse_transform(top_indices[i, :top_k])) # 添加top_k类别标签
    prob_data.append(prob_dict) # 添加到列表中
